...
Quarto.enhance_build do |build|
  build.extend(Quarto::Orgmode::BuildExt)
  build.orgmode = Quarto::Orgmode::Build.new(build)
  build.extensions_to_source_formats["org"] = "orgmode"
end

Quarto.define_tasks do
  namespace :orgmode do
    task :vendor => orgmode.vendor_dir
  end

  directory orgmode.vendor_dir =>
    "#{vendor_dir}/org-#{orgmode.version}.tar.gz" do |t|

    cd vendor_dir do
      sh "tar -xzf org-#{orgmode.version}.tar.gz"
    end
    cd orgmode.vendor_dir do
      sh "make"
    end
  end

  file "#{vendor_dir}/org-#{orgmode.version}.tar.gz" => vendor_dir do |t|
    cd vendor_dir do
      sh "wget http://orgmode.org/org-#{orgmode.version}.tar.gz"
    end
  end
end
